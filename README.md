...
### 1. 🎯 셀프 목표 설정 (Self Goal)
- 메인 화면에서 사용자가 직접 목표 거리와 목표 시간을 설정할 수 있습니다.
- 목표 페이스가 자동으로 계산되어 표시됩니다.
- 설정한 목표에 맞춰 맞춤형 훈련 플랜이 생성됩니다.

### 2. 📊 논문 기반 적응형 알고리즘 (Adaptive Training)
- **Jack Daniels' VDOT Running Formula** 기반의 과학적 훈련 계산
- 러닝 완료 시마다 실제 수행 능력(VDOT)을 자동 계산
- **페이스 개선 감지**: VDOT가 5% 이상 향상되면 자동으로 플랜 난이도 상향
- **훈련 누락 감지**: 5일 이상 빼먹거나 페이스가 저하되면 플랜 자동 재조정
- **점진적 조정**: 정상 범위 내에서는 이동평균으로 부드럽게 업데이트

### 3. AI 맞춤형 플랜 (Smart Plan)
- 사용자의 키, 체중(BMI), 10km 기록, 주간 목표 시간을 분석합니다.
- 입문(12주), 중급(24주), 상급(48주) 레벨에 맞춰 과학적인 훈련 스케줄을 자동으로 생성합니다.
- **피리어다이제이션(주기화)**: 3주 증가 + 1주 회복 사이클 적용
- 각 훈련마다 목표 페이스(이지런, 템포런, 인터벌) 자동 계산

### 4. 실시간 AI 보이스 코칭 (Live Coaching)
- **Gemini AI**가 탑재되어 있습니다.
- 러닝 중 1분마다 현재 페이스와 거리를 분석하여, 상황에 맞는 격려와 조언을 음성(TTS)으로 들려줍니다.
- 예: *"페이스가 너무 빠릅니다. 호흡을 조절하며 천천히 달리세요."*

### 5. 클라우드 기록 동기화 (Cloud Sync)
